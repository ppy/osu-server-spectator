...
namespace osu.Server.Spectator.Hubs.Multiplayer
{
    ...
    public class MultiplayerHubContext : IMultiplayerHubContext
    {
        ...
        {
            if (!room.PlaylistImplementation.CurrentItem.Freestyle)
            {
            ...
            else
            {
                ...
                {
                    itemBeatmap = (await db.GetBeatmapAsync(room.PlaylistImplementation.CurrentItem.BeatmapID))!;
            ...
            foreach (var user in room.Users)
            {
                if (!room.PlaylistImplementation.CurrentItem.ValidateUserMods(user, user.Mods, out var validMods))
        ...
        public async Task ChangeUserStyle(int? beatmapId, int? rulesetId, ServerMultiplayerRoom room, MultiplayerRoomUser user)
        {
            ...
            if (beatmapId != null || rulesetId != null)
            {
                ...
                using (var db = databaseFactory.GetInstance())
                {
                    database_beatmap itemBeatmap = (await db.GetBeatmapAsync(room.PlaylistImplementation.CurrentItem.BeatmapID))!;
        ...
        public async Task ChangeUserMods(IEnumerable<APIMod> newMods, ServerMultiplayerRoom room, MultiplayerRoomUser user)
        {
            ...
            if (!room.PlaylistImplementation.CurrentItem.ValidateUserMods(user, newModList, out var validMods))
        ...
        public async Task StartMatch(ServerMultiplayerRoom room)
        {
            ...
            await multiplayerEventLogger.LogGameStartedAsync(room.RoomID, room.PlaylistImplementation.CurrentItem.ID, room.MatchTypeImplementation.GetMatchDetails());
        }
